...
const ChevronLeftIcon = () => (
  <svg className="cf-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
export function CoverflowCarousel({
  ...
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // Past a few pixels this is a drag, not a tap, so the release must not
    // also be read as a click on whatever card sits under the pointer.
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    // A tap rather than a throw: bring whichever card was tapped to the centre.
    // The frame holds pointer capture, so the release event reports the frame
    // as its target — hit-test the release point to find the real card.
    if (!movedRef.current) {
      const hit = document.elementFromPoint(event.clientX, event.clientY);
      const card = hit?.closest?.("[data-slide]");
      const index = Number(card?.getAttribute("data-slide"));
  ...
  return (
    <div
      className={cn("cf-root", className)}
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                data-slide={index}
                className={cn("cf-card", cardClassName)}
                style={{ width: "var(--cf-card)" }}
              >
                <img src={slide.src} alt={slide.alt} draggable={false} className="cf-card-img" />
              </div>
            ))}
          </div>
        </div>
      ...
      {showCaption && active?.title && (
        <div key={selected} className="cf-caption">
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="cf-caption-meta">
              {active.meta.map((row) => (
                <div key={row.label} className="cf-caption-meta-row">
                  <dt className="cf-caption-meta-label">{row.label}</dt>
                  <dd className="cf-caption-meta-value">{row.value}</dd>
                </div>
              ))}
            </dl>
          )}
        ...
        <div className="cf-pagination">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn("cf-dot", index === selected && "cf-dot-active")}
            />
          ))}
        </div>
      )}
