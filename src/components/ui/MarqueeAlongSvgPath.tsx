...
/** One item riding the path. Its own component so each instance owns a stable
    set of hooks — calling these straight from a `.map()` in the parent would
    silently rely on the item count never changing between renders. */
const MarqueeItem = ({
  child,
  itemIndex,
  itemCount,
  ...
  return (
    <motion.div
      className={cn("marquee-path-item", draggable && grabCursor && "marquee-path-item-grab")}
      style={{
...
const MarqueeAlongSvgPath = ({
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      // A zero-sized box (hidden tab, section not laid out yet) would bake in
      // scale(0) permanently. Skip and wait for the observer to fire again.
    ...
    // A ResizeObserver rather than a window listener: the box can reach its
    // real size well after mount (hidden tab, late layout) without the window
    // ever resizing, and a one-shot measure would leave it stuck at scale 0.
    const wrapper = container.current;
    if (!wrapper) return;
  ...
  const items = React.useMemo(() => {
    ...
    return childrenArray.flatMap((child, childIndex) =>
      Array.from({ length: repeat }, (_, repeatIndex) => {
        ...
        return { child, repeatIndex, itemIndex, key };
      }),
    );
  }, [children, repeat]);
  ...
  return (
    <div
      ...
      className={cn("marquee-path-root", className)}
    >
      <div ref={marqueeContainerRef} className="marquee-path-inner" style={{ contain: "layout style" }}>
        <svg
